...
func Test_Generic(t *testing.T) {
	...
	t.Run("Test_Generic_Base", func(t *testing.T) {
	...
	t.Run("Test_LFU_Generic", func(t *testing.T) {
		// mockClock := clock.NewClock()
		port, err := internal.GetFreePort()
		if err != nil {
			t.Error(err)
	...
	// Certain commands will need to be tested in a server with an eviction policy.
	// This is for testing against an LRU eviction policy.
	t.Run("Test_LRU_Generic", func(t *testing.T) {
		// mockClock := clock.NewClock()
		port, err := internal.GetFreePort()
		if err != nil {
			t.Error(err)
